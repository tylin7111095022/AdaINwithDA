...
def main():
    ...
    optimizer = torch.optim.Adam(params,lr = args.init_lr,betas=(0.9,0.999))
    ##紀錄訓練的一些參數配置
    logging.info(f'''
    =======================================
...
def calc_multiloss(losses:list, log_vars:list, device):
  totalLoss = 0
  for k in range(len(losses)):
    l = losses[k].to(device)
    var = log_vars[k].to(device)
    precision = torch.exp(-var) # weight of loss
    # print(f"weight of {k}: {precision}")
    totalLoss += precision * l + var
  return totalLoss
